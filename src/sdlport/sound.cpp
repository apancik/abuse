...
// Anonymous namespace to limit scope of sound system state
namespace
{
...
int sound_init(int argc, char **argv)
{
    ...
    {
        printf("Sound: Initialization failed - %s\n", e.what());
        return 0;
    }
}
    ...
    : m_chunk(nullptr)
{
    ...
    if (!rw)
    {
        ...
        SDL_Log("Failed to open file %s: %s", full_path.c_str(), SDL_GetError());
        return;
    }
    ...
    if (!m_chunk)
    {
        ...
        SDL_Log("Failed to load WAV from file %s: %s", full_path.c_str(), Mix_GetError());
    }
}
...
song::song(char const *filename)
    : Name(filename ? strdup(filename) : nullptr) // Store filename copy
      ,
      ...
      music(nullptr) // SDL mixer music object
{
    ...
    try
    {
        ...
        const std::filesystem::path realname =
            std::filesystem::path(get_filename_prefix()) / filename;

        // Load HMI format music file into memory
        uint32_t data_size;
        data = load_hmi(realname.string().c_str(), data_size);

        if (!data)
        {
            printf("Sound: ERROR - could not load %s\n", realname.string().c_str());
            return;
        }
        ...
        if (!music)
        {
            printf("Sound: ERROR - %s while loading %s\n",
                   Mix_GetError(), realname.string().c_str());
        }
    }
    catch (const std::exception &e)
    {
...
song::~song()
{
    ...
    free(Name); // Using free because it was allocated by strdup
