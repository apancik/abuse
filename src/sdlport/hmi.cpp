...
namespace
{
    ...
    // Constants for MIDI format
    constexpr size_t MAX_NOTE_OFF_EVENTS = 30;
    constexpr uint32_t INVALID_TIME = 0xFFFFFFFF;
    constexpr uint32_t HMI_TRACK_DATA_OFFSET = 0x57; // Offset to actual MIDI data in track
    constexpr uint32_t HMI_TRACK_OFFSET_POS = 0xE8;  // Position of track offset table
    constexpr uint32_t HMI_NEXT_CHUNK_POS = 0xF4;    // Position of next chunk offset

    // MIDI file magic numbers
    constexpr std::array<uint8_t, 4> MIDI_HEADER_MAGIC = {0x4D, 0x54, 0x68, 0x64}; // "MThd"
    constexpr std::array<uint8_t, 4> MIDI_TRACK_MAGIC = {0x4D, 0x54, 0x72, 0x6B};  // "MTrk"

    /**
     * @brief Represents a pending note-off event in the MIDI stream
     */
    struct NoteOffEvent
    {
    ...
    /**
     * @brief Converts a little-endian 32-bit integer from a buffer
     */
    uint32_t get_int_from_buffer(const uint8_t *buffer)
    {
    ...
    /**
     * @brief Writes a 32-bit integer in big-endian format to a buffer
     */
    void write_big_endian_number(uint32_t value, uint8_t *buffer)
    {
    ...
    /**
     * @brief Writes a variable-length time value to MIDI data
     */
    void write_time_value(uint32_t time, uint8_t *&buffer)
    {
    ...
    /**
     * @brief Handles the conversion of a single HMI track to MIDI format
     */
    ...
    public:
        ...
        void convert_track(uint8_t *input, uint32_t input_size, uint8_t *&output)
        {
            uint8_t *const start_of_buffer = output;
            uint8_t *const start_of_input = input;

            // Skip to actual MIDI data
            input += input[HMI_TRACK_DATA_OFFSET];

            // Write track header (length will be updated later)
            std::copy(MIDI_TRACK_MAGIC.begin(), MIDI_TRACK_MAGIC.end(), output);
            output += 8;
            ...
            while (!done && static_cast<uint32_t>(input - start_of_input) < input_size)
            {
                ...
                // Handle command byte or running status
                ...
                process_note_off_events(current_time, last_time, output);

                // Write timing and command unless it's an HMI-specific event
                ...
                done = process_command(current_command, current_value, current_time, input, output);
            }

            // Add end of track marker if needed
            if (!done)
            {
                const std::array<uint8_t, 4> end_marker = {0x00, 0xFF, 0x2F, 0x00};
                std::copy(end_marker.begin(), end_marker.end(), output);
                output += 4;
            }
    ...
    private:
        ...
        void remember_note_off_event(uint32_t time, uint8_t cmd, uint8_t note)
        {
        ...
        void process_note_off_events(uint32_t current_time, uint32_t &last_time,
                                     uint8_t *&buffer)
        {
            for (auto &event : note_off_events)
            {
                ...
                *buffer++ = event.note;
                *buffer++ = 0x00; // Velocity for note-off
        ...
        bool process_command(uint8_t command, uint8_t value, uint32_t current_time,
                             uint8_t *&input, uint8_t *&output)
        {
            ...
            case 0x90: // Note on with duration
                *output++ = value;
                *output++ = *input++;
                remember_note_off_event(
                    current_time + read_time_value(input),
                    command,
                    value);
                break;

            case 0xF0: // Meta event or HMI specific
                if (command == 0xFE)
                {
                    process_hmi_event(value, input);
                }
                else
                {
                    *output++ = value;
        ...
        void process_hmi_event(uint8_t value, uint8_t *&input)
        {
            switch (value)
            {
            case 0x10:
...
uint8_t *load_hmi(char const *filename, uint32_t &data_size)
{
    ...
    // Open input file
    std::ifstream hmi_file(filename, std::ios::binary);
    if (!hmi_file)
        return nullptr;

    // Get file size
    hmi_file.seekg(0, std::ios::end);
    uint32_t buffer_size = static_cast<uint32_t>(hmi_file.tellg());
    hmi_file.seekg(0, std::ios::beg);

    // Read input file
    auto input_buffer = std::make_unique<uint8_t[]>(buffer_size);
    hmi_file.read(reinterpret_cast<char *>(input_buffer.get()), buffer_size);
    if (!hmi_file)
        return nullptr;

    // Allocate output buffer (MIDI can be larger than HMI)
    auto output_buffer = std::make_unique<uint8_t[]>(buffer_size * 10);
    uint8_t *output_ptr = output_buffer.get();
    ...
    // Write MIDI header
    const std::array<uint8_t, 14> midi_header = {
    ...
    std::copy(midi_header.begin(), midi_header.end(), output_ptr);
    output_ptr += midi_header.size();

    // Write tempo track
    const std::array<uint8_t, 19> tempo_track = {
    ...
    std::copy(tempo_track.begin(), tempo_track.end(), output_ptr);
    output_ptr += tempo_track.size();
    ...
    for (int i = 0; i < num_tracks; i++)
    {
        ...
        converter.convert_track(&input_buffer[track_position], track_size, output_ptr);
    }

    // Calculate final size and prepare result buffer
    data_size = static_cast<uint32_t>(output_ptr - output_buffer.get());
    uint8_t *final_buffer = static_cast<uint8_t *>(malloc(data_size));
    std::memcpy(final_buffer, output_buffer.get(), data_size);

    // Save the MIDI file alongside the HMI file
    // std::filesystem::path input_path(filename);
    // auto midi_path = input_path.parent_path() / input_path.stem().concat(".mid");

    // std::ofstream midi_file(midi_path, std::ios::binary);
    // if (midi_file)
    // {
    //     midi_file.write(reinterpret_cast<const char *>(final_buffer), data_size);
    // }
